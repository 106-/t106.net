class PointConvert {
  ...
  resize(width, height) {
    ...
    this.rev_convert_matrix = tf.tensor(
      math.inv(this.convert_marix.arraySync()),
    );
  }
}
...
class Points {
  constructor(points, width, color, pc) {
    this.points = tf.concat2d([points, tf.ones([points.shape[0], 1])], 1);
  ...
  constructor(x, mu, sigma, range, color, pc) {
    ...
    this.points = tf
      .stack([
        ...
        tf.concat([
          tf.add(mu, sigma_range),
          tf.reverse(tf.sub(mu, sigma_range)),
        ]),
        tf.ones([x.shape[0] * 2]),
      ])
      .transpose();
