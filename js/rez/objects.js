Ring = function (interval, inner, outer, separate_num) {
  ...
  for (var i = 0; i < 360; i++) {
    ...
    if (i % angle == 0) {
      this.inner_dots.push(
        new THREE.Vector3(
          Math.cos(radians(i)) * this.inner,
          Math.sin(radians(i)) * this.inner,
          0,
        ),
      );
      this.outer_dots.push(
        new THREE.Vector3(
          Math.cos(radians(i)) * this.outer,
          Math.sin(radians(i)) * this.outer,
          0,
        ),
      );
    }
  ...
  for (var i = 0; i < separate_num; i++) {
    ...
    this.group.add(
      new THREE.Mesh(this.vert_meshlines[i].geometry, this.material),
    );
  }
};
...
Ring.prototype.dots_update = function () {
  ...
  if (tick == 0) {
    for (var i = 0; i < this.separate_num; i++) {
      ...
      else this.outer_max[i] = this.outer - 2 + (Math.random() - 0.5) * 6;
    }
  }
  ...
  if (tick < maxtick) {
    for (var i = 0; i < this.separate_num; i++) {
      r =
        this.inner_min[i] +
        ((this.outer_max[i] - this.inner_min[i]) * tick) / maxtick;
